...
class Bounds {
	...
	extend(wire) {
		...
		wire.forEach(a => {
			var dir = a[0];
			var len = parseInt(a.substring(1));
			switch (dir) {
				case 'R': {
					pos.x += len
					break;
...
class Matrix {
	...
	bounds
	constructor(bounds) {
		this.map = new Array((bounds.maxY - bounds.minY) + 1).fill(0).map(a => new Array((bounds.maxX - bounds.minX) + 1).fill(Infinity));
		this.bounds = bounds;
	}
	draw(wire) {
		var pos = new Point();
		...
		wire.map(a => {
			...
			for (var i = 0; i < len; i++) {
				time++;
				pos[v] += inc;
				this.map[pos.y - this.bounds.minY][pos.x - this.bounds.minX] = Math.min(this.map[pos.y - this.bounds.minY][pos.x - this.bounds.minX], time);
			}
		})
		return this;
	}
	...
	intersect(matrix) {
		...
		for (var y = 0; y < this.map.length; y++) {
			for (var x = 0; x < this.map[0].length; x++) {
				if (this.map[y][x] !== Infinity && matrix.map[y][x] != Infinity) {
					intersect.push(new Intersect(x + this.bounds.minX, y + this.bounds.minY, this.map[y][x], matrix.map[y][x]));
				}
			}
		}
		return intersect;
...
function part1(input) {
	var wire1 = input[0];
	var wire2 = input[1];
	var bounds = new Bounds().extend(wire1).extend(wire2);
	var matrix1 = new Matrix(bounds).draw(wire1);
	var matrix2 = new Matrix(bounds).draw(wire2);
	return intersects = Math.min(...matrix1.intersect(matrix2).map(a => a.distance()));
}
