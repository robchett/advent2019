...
function part1(input) {
    ...
    input.forEach(s => {
        [prev, name] = s.split(")");
        if (!nodeList[prev]) {
            nodeList[prev] = new Node(prev);
        }
        if (!nodeList[name]) {
            nodeList[name] = new Node(name);
        }
        nodeList[name].prev = nodeList[prev];
        nodeList[prev].children.push(nodeList[name]);
    })
    var out = 0;
    nodeList.toArray().forEach(n => {
        do {
            n = n.prev;
            out++;
        } while (n);
    })
    return out - nodeList.toArray().length;
